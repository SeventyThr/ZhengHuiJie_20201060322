...
void matrix3x3SetIdentity(Matrix3x3 matIdent3x3) {//初始化
	GLint row, col;
	for (row = 0; row < 3; row++)
		for (col = 0; col < 3; col++)
...
void matrix3x3PreMultiply(Matrix3x3 m1, Matrix3x3 m2) {//矩阵乘法
	GLint row, col;
	Matrix3x3 matTemp;
	for (row = 0; row < 3; row++)
		for (col = 0; col < 3; col++)
...
void transformVerts2D(GLint nVerts, wcPt2D* verts) {//坐标变换？
	GLint k;
	GLfloat temp;
	for (k = 0; k < nVerts; k++) {
...
void displayFcn(void) {
	GLint nVerts = 3;
	wcPt2D verts[3] = { {75.0,50.0},{150.0,50.0},{100.0,100.0} };
	wcPt2D centroidPt;
